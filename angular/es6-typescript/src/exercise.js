...
function destructuringArray() {
    const names = ['Ashim', 'Suraj', 'Shekhar', 'Bhuwan', 'Ganesh'];
    // const names: string[] = [];
...
function restOperator() {
    ...
    // };
    const printName = (...items) => items.forEach(value => console.log(`Hello ${value}.`));
    printName();
...
function processTaggedTemplate() {
    const name = 'Ashim';
    const friend = function (strings, ...substitutions) {
        if (!substitutions[0]) {
