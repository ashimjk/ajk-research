...
import {AppState, currentProductSelector} from '../state/product-reducer';
import {select, Store} from '@ngrx/store';
import {InitializeCurrentProduct, SetCurrentProduct} from '../state/product.actions';
    ...
    if (this.product && this.product.id) {
      if (confirm(`Really delete the product: ${this.product.productName}?`)) {
        this.productService.deleteProduct(this.product.id).subscribe({
          next: () => this.store.dispatch(InitializeCurrentProduct.create()),
      ...
      if (this.productForm.dirty) {
        ...
        if (p.id === 0) {
          this.productService.createProduct(p).subscribe({
            next: product => this.store.dispatch(SetCurrentProduct.create(product.id)),
            error: err => this.errorMessage = err.error
          });
        } else {
